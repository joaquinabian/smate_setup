...
if [ -z "$PASSWRD" ]
then
    read -r -p "Wifi router archer password ?  " PASSWRD
fi
#
#
if [ "$RPITYPE" == "slave" ]
...
set_connection TP-Link_9780       $IP4 $GW4 "$DN4" $DH4 $DH6  10  wpa-psk "$PSW"
set_connection TP-Link_9780_5G    $IP4 $GW4 "$DN4" $DH4 $DH6  20  wpa-psk "$PSW"
...
echo "  1) Connect the PC to ARCHER WIFI (This will happens automatically)."
